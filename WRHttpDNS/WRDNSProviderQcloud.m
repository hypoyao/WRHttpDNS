...
@implementation WRDNSProvideQcloud


-(NSString *) getRequestString:(NSString *)domain isHttps:(BOOL)isHttps{
    NSString *urlString = [NSString stringWithFormat:@"%@://%@/d?dn=%@&ttl=1",(isHttps?@"http":@"https"),(isHttps?kHTTPDNS_QCLOUD_SERVER_ADDRESS:kHTTPDNS_HTTPS_QCLOUD_SERVER_ADDRESS),domain];
    return urlString;
}
